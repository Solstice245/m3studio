...
def draw_props(particle, layout):
    ...
    col.prop(particle, 'bounce', text='Bounciness')
    ...
    if particle.color_smoothing == 'LINEARHOLD' or particle.color_smoothing == 'BEZIERHOLD':
        col.prop(particle, 'color_hold', text='Color Hold Time')
        col.prop(particle, 'alpha_hold', text='Alpha Hold Time')
    ...
    sub = col.row(align=True)
    sub.prop(particle, 'uv_flipbook_col_fraction', text='Width/Height')
    sub.prop(particle, 'uv_flipbook_row_fraction', text='')
    col = layout.column()
    sub = col.row(align=True)
    sub.prop(particle, 'uv_flipbook_start_init_index', text='Phase 1 Start/End')
    sub.prop(particle, 'uv_flipbook_start_stop_index', text='')
    col.prop(particle, 'uv_flipbook_start_lifespan_factor', text='Phase 1 Length')
    sub = col.row(align=True)
